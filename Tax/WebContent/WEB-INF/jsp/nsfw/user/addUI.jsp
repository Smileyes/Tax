...
<body class="rightBody">
	<form id="form" name="form" action="nsfw/user_add.action" method="post"
				...
				<div class="content_info">
					...
					<table id="baseInfo" width="100%" align="center" class="list"
						...
						<tr>
							...
							<td><s:textfield name="user.account" id="account"
									onchange="verifyAccount()" /></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><s:radio list="#{'true':'男','false':'女'}"
									name="user.gender" value="true" /></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="tdBg " width="200px" id="datepicker">生日：</td>
						...
						<tr>
							...
							<td><s:radio list="#{'1':'有效','0':'无效'}" name="user.state"
									value="1" /></td>
						</tr>
						<tr>
...
<script type="text/javascript">
					...
					.ajax({
						...
						"success" : function(msg) {
							if (msg == "true") {
								$account.next("span").remove();
								$account
										.after("<span style='color: red;' class='addError'>该账号已存在，请修改帐号名称！</span>");
								$account.focus();
							} else {
								$account.next("span").remove();
								validate = true;
	...
	function beforeSubmit() {
		...
		if (!pwdReg.test($.trim(password))) {
			$(":text[name='user.password']")
					.after(
							"<span style='color: red;' class='addError'>密码位数应在6~15位之间，且只包含英文与数字</span>");
			canSubmit = false;
		}
		//检查角色
